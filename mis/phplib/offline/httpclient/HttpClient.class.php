...
class HttpClient {
    ...
    protected $response = null;//instance of class Response

    private static $curlDebug = false;//for verbose
    private static $maxRedirect = 0;//maximum amount of HTTP redirections
    private static $timeout = 15;//maximum number of seconds
    private static $connectTimeout = 1;//maximum number of seconds for connect
    private static $connectTimeoutMS = 1000;//maximum number of milliseconds for connect
    private static $timeoutMS = 15000;//maximum number of milliseconds
    
    private static $retryTimes = 2;//fail error for retry times
    private static $connnectRetryTimes = 3;//connect fail error for retry times
	...
	const errServiceInvalid = 2;	//对端服务不正常 connect timeout
...
//         ini_get('open_basedir') == '' && ini_get('safe_mode') == 'Off'
        ...
        defined('CURLOPT_CONNECTTIMEOUT_MS') && $this->curlOpt['CONNECTTIMEOUT_MS'] = self::$connectTimeoutMS;//The number of milliseconds to wait while trying to connect. 
        $this->curlOpt['TIMEOUT'] = self::$timeout;//The maximum number of seconds to allow cURL functions to execute. 
        defined('CURLOPT_TIMEOUT_MS') && $this->curlOpt['TIMEOUT_MS'] = self::$timeoutMS;//The maximum number of milliseconds to allow cURL functions to execute.
        ...
        $this->curlOpt['HEADERFUNCTION'] = array($this->response, 'headerCallback');//__CLASS__.'::getHeader';//The header data must be written when using this callback function
        $this->curlOpt['WRITEFUNCTION'] =  array($this->response, 'bodyCallback');//__CLASS__.'::getResult';//The data must be saved by using this callback function.
    ...
    public function options($optArr=NULL){
        if($optArr === NULL)
            return $this->curlOpt;
        $this->curlOpt = array_merge($this->curlOpt, (array)$optArr);
        return $this;
    }
...
//             throw new Exception('exception string notice....',1); //由接口逻辑决定如何处理异常
	...
	private function curlExec($method = self::GET) {
		...
		//changed for timeout optmizing by zhangfan06
		if (defined ( 'CURLOPT_TIMEOUT_MS' ) && defined ( 'CURLOPT_CONNECTTIMEOUT_MS' )) {
			if(isset($arr[CURLOPT_TIMEOUT_MS]) && $arr[CURLOPT_TIMEOUT_MS] > 0 && isset($arr[CURLOPT_CONNECTTIMEOUT_MS]) && $arr[CURLOPT_CONNECTTIMEOUT_MS] > 0){
				unset($arr[CURLOPT_TIMEOUT]);
				unset($arr[CURLOPT_CONNECTTIMEOUT]);
				$arr[CURLOPT_NOSIGNAL] = true;
			} else {//默认timeout及connecttimeout必须设置
				unset($arr[CURLOPT_CONNECTTIMEOUT_MS]);
				unset($arr[CURLOPT_TIMEOUT_MS]);
			}
		}
        ...
        do {
        	//connect timeout for retrying
	        $connectTimes = self::$connnectRetryTimes;
        	do {
	        	//reset msg
	        	$this->resetResult();
	        	curl_exec($ch);
	        	...
	        	if( $this->errno > 0) 
					CLogger::warning("serverUrl:{$this->serverUrl}, connectTimes:$connectTimes, errno:{$this->errno},errmsg: {$this->errmsg}");
        	} while(($this->errno == CURLE_COULDNT_CONNECT || ($this->errno == CURLE_OPERATION_TIMEOUTED && stripos($this->errmsg, 'Connection') !== false)) && --$connectTimes);
			
			if ($this->errno > 0) {//error for retry
				usleep(self::$sleepTime);//pause 100ms for later retry
				
				if (($this->httpCode == 301 || $this->httpCode == 302 || $this->httpCode == 307) && preg_match ('/Location: ([^\r\n]+)[\r\n]*/si', $this->response->responseHeader, $matches ) > 0) {
					$nextUrl = $matches[1];
...
// 			$this->errno = self::errOtherEror;
    ...
    public function xml_result($domObject=FALSE){
        if($domObject){
            ...
            $d->loadXML($this->response->responseBody);
            return $d;
        }else
            return simplexml_load_string($this->response->responseBody);
    }
    ...
    public function json_result($toArray=FALSE){
        return json_decode($this->response->responseBody,$toArray);
    }
...
class Response
{
	 ...
	 * @return successfully write length
	 */
    ...
    public function bodyCallback($ch, $body)
    {
