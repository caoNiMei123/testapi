...
class SQLTemplate implements ISQL
{
    const V_BEGIN = '{';
    const V_END = '}';
    const V_SPLIT = ':';
...
//        print_r($this->arrVar);
    ...
    */
    ...
    {
        $k = SQLTemplate::V_BEGIN.$k;
        if(!array_key_exists($k, $this->arrVar))
        {
            return null;
        }

        if($this->arrVar[$k] == SQLTemplate::T_NUM)
        {
            if(!is_numeric($v))
            {
                return false;
        ...
        else if($this->arrVar[$k] != SQLTemplate::T_RAW)
        {
    ...
    public function __parse()
    {
        ...
        {
            // search for variable
            $nextOff = strpos($this->template, SQLTemplate::V_BEGIN, $off);
            // not found
            if($nextOff === false)
            {
            ...
            $nextOff = strpos($this->template, SQLTemplate::V_SPLIT, $off);
            // bad format
            if($nextOff === false || ($varLen = $nextOff - $off) == 1 ||
                $this->template{$nextOff+2} !=  SQLTemplate::V_END)
            {
                $this->arrParsed = NULL;
                $this->arrVar = NULL;
                return false;
    ...
    private function __createSQL()
    {
        ...
        {
            if(strncmp($v, SQLTemplate::V_BEGIN, strlen(SQLTemplate::V_BEGIN)) == 0)
            {
